...
angular.module('google.places', [])
  ...
  .directive('gPlacesAutocomplete',
                ...
                link: function ($scope, element, attrs, controller) {
                    ...
                    function initAutocompleteDrawer() {
                        ...
                        var drawerElement = angular.element('<div data-tap-disabled="true" g-places-autocomplete-drawer></div>'),
                            body = angular.element($document[0].getElementById("placeautocomplete")),
                            $drawer;
    ...
    .directive('gPlacesAutocompletePrediction', [function () {
        var TEMPLATE = [
            '<small class="pac-icon pac-icon-marker"></small>',
            '<small class="pac-item-query" data-tap-disabled="true" ng-bind-html="prediction | highlightMatched"></small>',
            '<small ng-repeat="term in prediction.terms | unmatchedTermsOnly:prediction">{{term.value | trailingComma:!$last}}&nbsp;</small>',
            '<small class="custom-prediction-label" ng-if="prediction.is_custom">&nbsp;{{prediction.custom_prediction_label}}</small>'
        ];
    ...
    .filter('highlightMatched', ['$sce', function ($sce) {
        return function (prediction) {
            ...
            return $sce.trustAsHtml('<small class="pac-matched">' + matchedPortion + '</small>' + unmatchedPortion);
        };
    }])
